...
bool recursion_step(Schedule& sched, int index, Turno** turn_list, int turn_length, int total_slots, long long node_budget) {
	...
	if (node_budget > 0 && nodes_visited > node_budget) return false;

	int num_employees = (int)sched.employees.size();

	// Skip constrained slots transparently.
	// Day-major layout: day = index / num_employees, emp_index = index % num_employees.
	// Filling everyone's Monday before anyone's Tuesday means is_day_filled_correctly
	// (checked below on the last employee of a day) fires after ~num_employees slots
	// instead of after nearly the whole schedule, so bad-day dead ends are caught and
	// backtracked far earlier.
	while (index < total_slots) {
		int day = index / num_employees;
		int emp_index = index % num_employees;
	...
	// Base case: all slots filled. Under day-major order, no single employee's week
	// is fully known until every day has been processed, so per-employee validity
	// (min_ore, min_rest) can only be checked here, across everyone, rather than
	// incrementally at an employee boundary.
	if (index == total_slots) {
		for (auto& emp : sched.employees) {
			if (!emp.is_emp_week_valid()) return false;
		}
		return true;
	}
	...
	for (int i = 0; i < turn_length; i++) {
		...
		bool ok = true;
		if (emp_index == num_employees - 1) {
			if (!sched.is_day_filled_correctly((enum days)day, turn_list, turn_length)) {
				ok = false;
			}
		}

		if (ok && recursion_step(sched, index + 1, turn_list, turn_length, total_slots, node_budget)) {
			return true;
		}
...
int main() {
	...
	const long long NODE_BUDGET = 2000000; // per-attempt cap before reshuffling and retrying
	...
	for (int attempt = 1; attempt <= MAX_ATTEMPTS && !found; attempt++) {
		Schedule attempt_sched = sched; // reset to the CSV-constrained baseline each attempt
		nodes_visited = 0;
		max_index = 0;

		cout << endl << "--- Attempt " << attempt << " (node budget " << NODE_BUDGET << ") ---" << endl;

		found = recursion_step(attempt_sched, 0, &turni[0], (int)turni.size(), total_slots, NODE_BUDGET);

		if (found) {
			best_sched = attempt_sched;
		}
	}

	if (!found) {
		cout << endl << "No solution found after " << MAX_ATTEMPTS << " attempts." << endl;
		return 1;
	}
